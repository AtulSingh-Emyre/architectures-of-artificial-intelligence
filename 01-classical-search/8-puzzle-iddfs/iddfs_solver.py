...
def depth_limited_search(game_env: BaseEnvironment, current_puzzle_sate: tuple[S,I],depth:int) -> tuple[bool,list]:
    ...
    # check if depth is 0 then return false
        ...
        for next_puzzle_state in next_puzzle_states:
            curr_branch = depth_limited_search(game_env,next_puzzle_state,depth-1)
            if curr_branch[0]:
...
def iddfs(game_env: BaseEnvironment) -> tuple[bool,list]:
    ...
    while True:
        ...
        current_result = depth_limited_search(game_env, current_puzzle_sate=game_env.get_initial_puzzle_state(),depth=D)
        if current_result[0]:
